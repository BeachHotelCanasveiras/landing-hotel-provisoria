...
async function detectGeoLanguage(): Promise<string | null> {
  try {
    const response = await Promise.race([
      ...
      timeoutSignal(1500) // Límite de seguridad
    ]) as Response;
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        let targetLang = StorageService.getCookie(LANG_COOKIE);
        ...
        // Captura cualquier excepción para evitar congelamiento de la UI
        console.error('[useAppMiddleware] Error crítico durante la inicialización:', error);
      } finally {
        // 7. LIBERACIÓN INCONDICIONAL: Garantiza que la UI cargue bajo cualquier escenario
        setIsReady(true);
      }
    };
